...
// Purpose: Parameterized PE-grid int8/int32 tile multiply block for the first MXU milestone.
// Public TPU inspiration: TPU-like accelerators center matrix work around MXU-style PE arrays.
// Educational simplification: The grid is output-stationary and advances one K slice at a time.
// Inputs: start, accumulate, flattened A/B/C tiles.
// Outputs: flattened C tile plus busy/done/error status.
// State: IDLE -> FEED -> CAPTURE -> DONE or ERROR.
// Latency: Two cycles per K slice plus command/control overhead.
// Backpressure: start is accepted only when busy is low.
// Error behavior: zero or oversized dimensions raise error.
// Tests: MXU/chip cocotb tests compare exact NumPy int8/int32 behavior.
module systolic_array #(
  ...
  logic signed [ACC_W-1:0] acc_q [0:ARRAY_M*ARRAY_N-1];
  logic signed [ACC_W-1:0] pe_acc_out [0:ARRAY_M*ARRAY_N-1];
  logic signed [DATA_W-1:0] pe_a_out [0:ARRAY_M*ARRAY_N-1];
  logic signed [DATA_W-1:0] pe_b_out [0:ARRAY_M*ARRAY_N-1];
  integer comb_idx;
  integer reset_idx;
  integer start_idx;
  integer capture_idx;
          ...
          if (start) begin
            if ((m == 16'd0) || (n == 16'd0) || (k == 16'd0) ||
              ...
              for (start_idx = 0; start_idx < ARRAY_M*ARRAY_N; start_idx++) begin
                acc_q[start_idx] <= accumulate ? c_in[start_idx] : '0;
              end
              k_idx_q <= 16'd0;
              state_q <= ST_FEED;
            end
