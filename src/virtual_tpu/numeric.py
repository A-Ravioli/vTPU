# numeric helpers for the golden model: int32 wrapping and bf16 matmul
...
def wrap_i32(values: np.ndarray) -> np.ndarray:
    """reduce int64 intermediates back to signed int32 (hardware-style overflow)."""
...
def float32_to_bf16(values: np.ndarray) -> np.ndarray:
    """round f32 to bf16 bit patterns stored as uint16."""
...
def bf16_to_float32(values: np.ndarray) -> np.ndarray:
    """expand bf16 uint16 bit patterns to f32 for numpy math."""
...
def bf16_matmul(a_bf16: np.ndarray, b_bf16: np.ndarray) -> np.ndarray:
    """c = a @ b via f32 intermediates; inputs/outputs are bf16 bit patterns."""
